...
console.log([API] Fetching from YouTube API: 
𝑡
𝑦
𝑝
𝑒
...
slide.innerHTML = <div class="video-preloader" style="background-image: url('${thumbnailUrl}');"><div class="loader"></div></div> ${playerHtml} <div class="video-meta-overlay"> <div class="uploader-info" onclick="${creatorProfileOnClick}"><img src="${uploaderAvatar}" class="uploader-avatar"><span class="uploader-name">${escapeHTML(uploaderName)}</span></div> <p class="video-title">${escapeHTML(title)}</p> <button class="add-channel-btn haptic-trigger" onclick="${addChannelOnClick}"><i class="fas fa-plus"></i> Add</button> </div>;
...
"
>
escapeHTML(video.snippet.title)</span><spanclass="long−video−uploader">
{escapeHTML(video.snippet.channelTitle)}</span> </div> </div> <button class="add-channel-btn-long-grid haptic-trigger" onclick="${addChannelOnClick}"><i class="fas fa-plus"></i> Add Channel</button>;
return card;
...
contentArea.innerHTML = <div id="creator-page-player-container"> <div class="player-back-button haptic-trigger" onclick="navigateBack()"><i class="fas fa-arrow-left"></i></div> <div class="player-wrapper"> <div id="creator-page-player-long"></div> </div> <button id="player-rotate-btn" class="haptic-trigger"><i class="fas fa-sync-alt"></i> Rotate</button> </div>;
...
const message = Shubhzone Withdrawal Request: -------------------------- User UID: 
𝑎
𝑝
𝑝
𝑆
...
:
₹
appState.currentUser.uidAmount:₹
{amount.toFixed(2)} UPI ID: ${upiId} Mobile No: ${mobileNo} --------------------------;
